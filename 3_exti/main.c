...
int main(void)
{
	...
	CLEAR_BIT(GPIO_BASE(GPIO_PORTA) + GPIOx_PUPDR_OFFSET, MODERy_0_BIT(0));
	CLEAR_BIT(GPIO_BASE(GPIO_PORTA) + GPIOx_PUPDR_OFFSET, MODERy_1_BIT(0));
	...
	//??????
	WRITE_BITS(SYSCFG_BASE + SYSCFG_EXTICR1_OFFSET, EXTI0_3_BIT, EXTI0_0_BIT, 0b00);

	//EXTI Configurations
	//??????
	SET_BIT(EXTI_BASE + EXTI_IMR_OFFSET, 0);
	CLEAR_BIT(EXTI_BASE + EXTI_RTSR_OFFSET, 0); //Rising=0
	SET_BIT(EXTI_BASE + EXTI_FTSR_OFFSET, 0);   //Falling
